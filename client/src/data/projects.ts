 ...
 * and a reference to its frame URL module.
 */
...
export interface ProjectData {
  ...
  disclaimer?: string;
  /** Fraction of image width that is baked-in black border on each side (e.g. 0.123). Source crop + contain mode shows full content with matching bg on sides. */
  contentCropX?: number;
  /** Explicit background color for the container. Fills any gaps from contain mode. */
  animBgColor?: string;
  /** How the frame image fits the canvas: 'cover' (default, fills viewport, may crop) or 'contain' (shows full image, may have bg bars) */
  fitMode?: 'cover' | 'contain';
  /** Gallery images for non-animated projects (CDN URLs) */
...
export const projects: ProjectData[] = [
  ...
  },
  {
    ...
    longDescription:
      "The Flatbed Modular Test Fixture is a precision test station designed for comprehensive electrical verification of EMG wristband assemblies in a flattened configuration. The fixture features a black anodized aluminum frame supported by spider-leg adjustable supports for leveling and stability. A dense pogo pin array makes contact with the EMG band's electrode pads when the band is laid flat across the test surface. Toggle switches provide manual control of test channels, while the modular PCB interface routes signals to automated test equipment. The design accommodates multiple EMG band variants through interchangeable nest inserts, and the open-frame architecture allows visual inspection during testing.",
