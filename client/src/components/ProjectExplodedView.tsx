 ...
 *
 * Dynamically samples the first frame's edge color to set the container
 * background, so any aspect-ratio gaps blend seamlessly with the image.
 */
...
/**
 * Sample the dominant background color from an image.
 */
  ...
  const drawFrame = useCallback(
    (frameIndex: number) => {
      ...
      if (cropX > 0) {
        ...
        // Fill background first to prevent black bars
        const bgC = animBgColor ?? bgColor;
        ctx.fillStyle = bgC;
        ctx.fillRect(0, 0, w, h);
        ...
        if (fitMode === 'contain') {
          // Contain: show full image, add background bars on sides
          if (imgAspect > canvasAspect) {
            // Image is wider than canvas — fit to width, bars top/bottom
            drawW = w;
            drawH = w / imgAspect;
            drawX = 0;
            drawY = (h - drawH) / 2;
          } else {
            // Image is taller than canvas — fit to height, bars left/right
            drawH = h;
            drawW = h * imgAspect;
            drawX = (w - drawW) / 2;
            drawY = 0;
  ...
  return (
    <section ref={sectionRef} className="relative" style={{ height: "500vh" }}>
      ...
      >
        ...
        {/* Header overlay with gradient background for readability */}
        <div
